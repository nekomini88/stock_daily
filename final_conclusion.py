...
def _determine_phase(sp_chg, qq_chg, ru_chg, pct2) -> str:
    """市场阶段判定 (原 analyze_session 内联逻辑)"""
    if pct2(sp_chg) > 1.5 and pct2(qq_chg) > 1.5:
        return "强多头"
    if pct2(sp_chg) > 0.5 and pct2(qq_chg) > 0:
        return "温和多头"
    if pct2(sp_chg) < -1.5 and pct2(qq_chg) < -1.5:
        return "剧烈调整"
    if pct2(sp_chg) < -0.5 and pct2(qq_chg) < 0:
        return "偏弱调整"
    if pct2(ru_chg or 0.0) > 0.5 and pct2(sp_chg or 0.0) < 0:
        return "高低切/补涨"
    if abs(pct2(sp_chg)) <= 0.3 and abs(pct2(qq_chg)) <= 0.5:
        return "窄幅震荡"
...
def _determine_bias(sp_chg, qq_chg, ru_chg, best_mag7, worst_sector, pct2) -> str:
    """交易倾向判定 (原 analyze_session 内联逻辑)"""
    good = bad = 0
    if pct2(sp_chg) > 0:
        good += 1
    else:
    ...
    if bad >= good:
        return "防守为主，优先减仓高风险成长股，关注公用事业、必需消费品等防御板块"
    if good == 0:
        return "维持现有仓位，不宜追涨杀跌，观察量能变化"
    return "偏进取，优先关注主线龙头，仓位可向强势板块倾斜，但避免单票仓位过重"


def _build_conclusion(sp_chg, qq_chg, soxx_chg, ru_chg, sp_name, qq_name,
                      bond, dxy, gold, top_sector, worst_sector,
                      best_mag7, worst_mag7, sector_info, pct2, _fmt) -> str:
    """市场结论组装 (原 analyze_session 内联逻辑)"""
    parts = []
    if pct2(sp_chg or 0) > 0 and pct2(qq_chg or 0) > 0:
...
def _build_signals(sp_chg, soxx_chg, ru_chg, top_sector, best_mag7,
                   bond, gold, themes_performance, sector_info, pct2, _fmt) -> List[str]:
    """信号池生成 (原 analyze_session 内联逻辑)"""
    signals: List[str] = []
    ...
    if pct2(sp_chg or 0) > 1.0:
        add_signal("标普强势大涨，确认短期多头格局延续")
    elif pct2(sp_chg or 0) < -1.0:
        add_signal("标普跌幅超过1%，短期风险升温")
    if soxx_chg is not None:
        if soxx_chg < -5.0:
    ...
    seen = set()
    return [s for s in signals if s not in seen and not seen.add(s)][:5]
...
def analyze_session(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # phase
    market_phase = _determine_phase(sp_chg, qq_chg, ru_chg, pct2)

    # bias
    trading_bias = _determine_bias(sp_chg, qq_chg, ru_chg, best_mag7, worst_sector, pct2)

    # 结论
    market_conclusion = _build_conclusion(
        sp_chg, qq_chg, soxx_chg, ru_chg, sp_name, qq_name,
        bond, dxy, gold, top_sector, worst_sector,
        best_mag7, worst_mag7, sector_info, pct2, _fmt)

    # 信号池
    signals = _build_signals(
        sp_chg, soxx_chg, ru_chg, top_sector, best_mag7,
        bond, gold, themes_performance, sector_info, pct2, _fmt)
